...
function saveToHistory(rockData) {
    const history = getHistory();
    const filtered = history.filter(h => h.name_ko !== rockData.name_ko);
    const newItem = Object.assign(Object.assign({}, rockData), { savedAt: new Date().toLocaleDateString('ko-KR', { hour: '2-digit', minute: '2-digit' }) });
...
function toggleHistoryList() {
    ...
    history.forEach((h, idx) => {
      ...
      <div class="history-item" onclick="loadHistoryItem(${idx})">
        <span class="history-name">${h.name_ko} <small style="font-weight:normal;color:#888;">(${((_a = h.classification) === null || _a === void 0 ? void 0 : _a.rock_type) || '암석'})</small></span>
...
function loadDictItem(key) {
    if (OFFLINE_ROCKS[key]) {
        const data = JSON.parse(JSON.stringify(OFFLINE_ROCKS[key]));
        if (userLocation) {
...
function getGPSLocation() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        navigator.geolocation.getCurrentPosition((position) => __awaiter(this, void 0, void 0, function* () {
            ...
            status.classList.remove('hidden');
            try {
                const res = yield fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&zoom=14`, {
                    headers: { 'Accept-Language': 'ko' }
                });
                if (res.ok) {
                    const data = yield res.json();
                    const address = data.display_name || ((_a = data.address) === null || _a === void 0 ? void 0 : _a.road) || ((_b = data.address) === null || _b === void 0 ? void 0 : _b.suburb) || ((_c = data.address) === null || _c === void 0 ? void 0 : _c.city) || '';
...
function saveKey() {
    ...
    if (!k.startsWith('AIzaSy')) {
        status.textContent = '⚠️ AIzaSy 로 시작하는 Gemini 키를 입력해 주세요.';
        return;
    }
...
if (uploadBox && fileInput) {
    ...
    fileInput.onchange = (e) => {
        ...
        const f = (_a = target.files) === null || _a === void 0 ? void 0 : _a[0];
        if (!f)
            return;
...
function startCam() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            stream = yield navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment', width: { ideal: 1280 }, height: { ideal: 720 } } });
            ...
            setTimeout(() => {
                try {
                    ...
                    if (capabilities.zoom && slider) {
                        ...
                        slider.value = String(videoTrack.getConstraints().zoom || 1);
            ...
            showErr('카메라 접근 권한이 필요하거나 후면 카메라를 활성화할 수 없습니다.');
        }
    });
}
    ...
    if (videoTrack) {
        try {
            ...
            if (caps.zoom) {
                ...
                }).catch(err => console.warn("applyConstraints zoom error", err));
            }
            else {
                const video = document.getElementById('video');
                if (video) {
...
function analyze() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const rockInputEl = document.getElementById('rockInput');
        if (!rockInputEl)
            return;
        const name = rockInputEl.value.trim();
        if (!name && !imgB64) {
            showErr('암석 이름을 입력하거나 이미지를 제공해 주세요.');
            return;
        }
        hideErr();
  ...
  `;
        ...
        window.scrollTo(0, 0);
        // 오프라인 검색 폴백 헬퍼
        const tryOfflineFallback = (nameInput) => {
            if (!nameInput)
                return null;
        ...
        if (!navigator.onLine) {
            ...
            if (localRock) {
                const data = JSON.parse(JSON.stringify(localRock));
                if (userLocation) {
                    data.location_compare = {
                        region_name: userLocation.address || "현재 분석 위치",
                        regional_rocks: [data.name_ko, "편마암", "화강암"],
...
{"name_ko":"한글이름","name_en":"English name","minerals":[{"name":"장석","percent":40},{"name":"석영","percent":30},{"name":"흑운모","percent":15},{"name":"휘석","percent":5},{"name":"감람석","percent":3}],"chemistry":[{"component":"SiO₂","percent":"72%"},{"component":"Al₂O₃","percent":"14%"},{"component":"K₂O","percent":"5%"},{"component":"Na₂O","percent":"4%"},{"component":"CaO","percent":"2%"},{"component":"Fe₂O₃","percent":"2%"}],"characteristics":{"color":"색상","hardness":"경도(모스)","texture":"조직","luster":"광택","origin":"생성환경"},"classification":{"rock_type":"화성암/퇴적암/변성암","sub_type":"세부분류","use":"용도별 분류"},"distribution":["분포지1","분포지2","분포지3"],"description":"설명 2-3문장"${userLocation ? ',"location_compare":{"region_name":"지역명 또는 산 이름","regional_rocks":["암석1","암석2"],"geology_summary":"지질학적 특징 요약","match_percent":85,"match_level":"매우 높음/보통/낮음","comparison_desc":"현재 암석과의 비교 및 발견 가능성에 대한 지질학적 설명"}' : ''}}`;
        ...
        try {
            ...
            if (!modelName) {
                const mRes = yield fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${apiKey}`);
                if (!mRes.ok)
                    throw new Error('API 키 검증 실패 또는 모델 목록을 가져올 수 없습니다.');
                const mData = yield mRes.json();
                ...
                if (!modelName)
                    modelName = models.find((m) => m.includes('vision') || m.includes('1.5')) || models.find((m) => m.includes('gemini'));
                if (!modelName)
                    throw new Error('지원되는 Gemini 모델이 없습니다. 허용된 모델: ' + models.join(', '));
                localStorage.setItem('rock_model_name', modelName);
            }
            const res = yield fetch(`https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${apiKey}`, {
                method: 'POST',
            ...
            if (!res.ok) {
                if (res.status === 404)
                    localStorage.removeItem('rock_model_name');
                const e = yield res.json().catch(() => ({}));
                throw new Error((((_a = e === null || e === void 0 ? void 0 : e.error) === null || _a === void 0 ? void 0 : _a.message) || `HTTP ${res.status}`) + ` (선택된 모델: ${modelName})`);
            }
            const data = yield res.json();
            const txt = ((_f = (_e = (_d = (_c = (_b = data.candidates) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.content) === null || _d === void 0 ? void 0 : _d.parts) === null || _e === void 0 ? void 0 : _e[0]) === null || _f === void 0 ? void 0 : _f.text) || '';
...
function svgBond(d) {
    var _a, _b, _c, _d;
    const rockType = ((_a = d.classification) === null || _a === void 0 ? void 0 : _a.rock_type) || '';
    const subType = ((_b = d.classification) === null || _b === void 0 ? void 0 : _b.sub_type) || '';
    const texture = ((_c = d.characteristics) === null || _c === void 0 ? void 0 : _c.texture) || '';
    const origin = ((_d = d.characteristics) === null || _d === void 0 ? void 0 : _d.origin) || '';
    ...
    const nodes = top.map((m, i) => {
        ...
        return Object.assign({ x: cx + rx, y: cy + ry }, m);
    });
    let lines = '';
    if (isI) {
...
function showResult(d) {
    ...
    if (d.description)
        html += card('📝 설명', `<p style="font-size:13px;color:#444;line-height:1.7">${d.description}</p>`);
    ...
    if (d.minerals)
        html += card('🔬 조암광물 비율', svgPie(d.minerals) + svgBar(d.minerals));
    if (d.chemistry) {
    ...
    if (d.minerals)
        html += card('🔗 결합구조도', svgBond(d) + `<p style="font-size:11px;color:#8a6a3a;margin-top:8px;text-align:center">주요 조암광물 간의 결합 관계 및 비율 시각화</p>`);
    ...
    if ((_a = d.distribution) === null || _a === void 0 ? void 0 : _a.length) {
...
window.toggleOfflineDict = toggleOfflineDict;
window.loadHistoryItem = loadHistoryItem;
window.loadDictItem = loadDictItem;
